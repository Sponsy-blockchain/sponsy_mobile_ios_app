...
class DetailedEventViewController : UIViewController, UIScrollViewDelegate {
    ...
    var event_id = -1
    var event_data_detailed:SponsyEventDetailed?
    var event_title = ""
    ...
    @IBOutlet var media_text:UILabel!
    @IBOutlet var gender_text:UILabel!
    ...
    override func viewDidLoad() {
        ...
        back_button.addTarget(self, action: #selector(DetailedEventViewController.backButtonPressed(_:)), for: .touchUpInside)
   ...
   func scrollViewDidScroll(_ scrollView: UIScrollView) {
    ...
    func shootHint() {
        if !UserDefaults().bool(forKey: "tutorial_event_passed") {
            ...
            if bottom_button_rect.midY <= view.bounds.height {
                NotificationCenter.default.post(name: Notification.Name.init("notification_show_hint"), object: nil, userInfo: ["hintText":"Send request to sponsor events you like. This is the first step towards landing a sponsorship deal.\nIn order to do this, you must have a sponsor profile setup in Sponsy website interface.","transparentHole":NSValue.init(cgRect: send_request_button.convert(send_request_button.bounds.insetBy(dx: -6, dy: -6), to: UIApplication.shared.keyWindow!)),"hintTappedHandler":{
                    ...
                    NotificationCenter.default.post(name: Notification.Name.init("notification_show_hint"), object: nil, userInfo: ["hintText":"Use this button to put a potential sponsorship deal between you-sponsor and this event-sponsee to public voting. People will vote about the appropriateness of this partnership.\nIn order to do this, you must have a sponsor profile setup in Sponsy website interface.","transparentHole":NSValue.init(cgRect: voting_button_hole),"hintTappedHandler":NSNull()])
                    UserDefaults().set(true, forKey: "tutorial_event_passed")
                    }])
            }
        }
    }
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        let event_task = HailyReceiveTask(dataOrigin: HailyDataOrigin.event, dataType: HailyDataType.details)
        event_task.event_id = event_id
        task_builder.addTasks([event_task])
        task_builder.sendTasksWithCompletionHandler({
            ...
            if let responses = parsedResponse {
                if let event_data = responses[0].eventDataDetailed {
                    success = true
                    DispatchQueue.main.async(execute: {
                        self.setEventData(event_data)
                        
                    })
                }
            }
            if let _error = error {
                print("error with populating detail event with event id \(self.event_id)")
                //maybe should pop view controller?
                print(_error)
            }
            if !success {
                DispatchQueue.main.async(execute: {
                    self.setEmptyStateWithText("Troubles while loading this topic")
        ...
        General.events_images_cache.retrieveImage(forKey: "\(event_id)", options: nil, completionHandler: {
            ...
            if let img = image {
                ...
                ImageDownloader.default.downloadImage(with: URL.init(string: "\(General.images_bucket_address)event\(self.event_id)")!, options: nil, progressBlock: nil, completionHandler: {
                    ...
                    if let img = image {
                        ...
                        General.events_images_cache.store(img, original: data, forKey: "\(self.event_id)")
                    }
                })
            }
        })
    ...
    func prepareWith(eventId:Int, eventTitle:String) {
        event_id = eventId
    ...
    func setEventData(_ data:SponsyEventDetailed) {
        UIView.animate(withDuration: 0.5, animations: {
            self.data_view.alpha = 1.0
        })
        self.event_data_detailed = data
        money_required_text.text = "$\(data.money_info)"
        date_text.text = data.date_info
        location_text.text = data.location_info
        audience_text.text = data.audience_info
        age_text.text = data.age_info
        media_text.text = data.media_info
        gender_text.text = data.gender_info
    ...
    func positionTopicTitle() {
        ...
        topic_title_label.text = event_title
    ...
    @IBAction func sendRequestPressed(sender:UIButton) {
        ...
        if General.authorized {
            ...
            send_request_vc.prepareWithRequestDestination("event", destinationTitle: event_title, destinationId: event_id)
    ...
    @IBAction func putForVotingPressed(sender:UIButton) {
        print("put pressed")
        if General.authorized {
            let voting_alert = UIAlertController.init(title: "Put for a vote", message: "You are about to make your potential partnership with this event available for public voting. This action requires having an active sponsor position. You can create one in an online interface.", preferredStyle: .alert)
            voting_alert.addAction(UIAlertAction.init(title: "Proceed", style: .default, handler: {
                (act:UIAlertAction) in
                let vote_task = HailyPutForVotingTask(partyId: self.event_id, partyType: "event")
